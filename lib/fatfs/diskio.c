...
static u8 *const cache_data = (u8 *)CONFIG_FATFS_CACHE_ADDR; /* in SDRAM */
...
)
{
	...
	while (count) {
		...
		if (!CACHE_IS_VALID(sector)) {
			...
			if (sdmmc_blk_read(&card0, &cache_data[chunk * FF_MIN_SS], chunk, FATFS_CACHE_SECTORS_PER_BIT) !=
				FATFS_CACHE_SECTORS_PER_BIT) {
