...
static int of_get_token_nextoffset(void *blob, int startoffset, int *nextoffset, unsigned int *token) {
	...
	*nextoffset = -1;
	...
	if (offset % 4) {
		info("DT: the token offset is not aligned\n");
		return -1;
	}
...
static int of_set_property(void *blob, int nodeoffset, const char *property_name, void *value, int valuelen) {
	...
	if (ret) {
		...
		if (ret)
			message("DT: fail to add property\n");
	...
	if (ret) {
		message("DT: fail to update property\n");
		return ret;
	}
...
int fixup_chosen_node(void *blob, char *bootargs) {
	...
	if (ret) {
		message("DT: doesn't support add node (chosen)\n");
		return ret;
	}
	...
	if (ret) {
		message("fail to set bootargs property\n");
		return ret;
	}
...
int fixup_memory_node(void *blob, unsigned int *mem_bank, unsigned int *mem_bank2, unsigned int *mem_size) {
	...
	if (ret) {
		info("DT: doesn't support add node (memory)\n");
		return ret;
	}
	...
	if (ret) {
		info("DT: could not set device_type property\n");
		return ret;
	}
	...
	if (ret) {
		info("DT: could not set reg property\n");
		return ret;
	}
